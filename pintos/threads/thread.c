...
fp_to_int_nearest (int x) {
	if (x >= 0)
		return (x + FP_FACTOR / 2) / FP_FACTOR;
	else
		return (x - FP_FACTOR / 2) / FP_FACTOR;
}
...
mlfqs_update_load_avg_and_recent_cpu (void) {
	...
	load_avg = fp_add (fp_mul (fp_div_int (fp (59), 60), load_avg),
			fp_mul_int (fp_div_int (fp (1), 60), ready_threads));
	...
	for (struct list_elem *e = list_begin (&all_list);
		...
		if (t == idle_thread)
			continue;
		coef = fp_div (fp_mul_int (load_avg, 2),
...
mlfqs_update_priority (struct thread *t) {
	...
	if (t == idle_thread)
		return;
	priority = PRI_MAX - fp_to_int_zero (fp_div_int (t->recent_cpu, 4))
		- t->nice * 2;
	if (priority > PRI_MAX)
		priority = PRI_MAX;
